...
const ComplaintCard = ({ id, name, description, createdDate, status, amount }) => {
  const navigation = useNavigation();

  const handlePress = () => {
    navigation.navigate('CloseComplaint', {
      complaint: { id, name, description, createdDate, status, amount },
    });
  };

  return (
    <TouchableOpacity onPress={handlePress} style={[styles.card, { borderColor: getStatusBorderColor(status) }]}>
      ...
      <Text style={styles.units}>Amount: {amount}</Text>
